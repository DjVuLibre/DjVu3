...
{
  if (!initialized)
    G_THROW("DjVuFile is not initialized");
}
...
DjVuFile::init(ByteStream & str)
{
   ...
   if (initialized)
     G_THROW("DjVuFile is already initialized");
   if (!get_count())
     G_THROW("DjVuFile is not secured by a GP<DjVuFile>");
...
DjVuFile::init(const GURL & xurl, GP<DjVuPort> port) 
{
   ...
   if (!get_count())
      G_THROW("DjVuFile is not secured by a GP<DjVuFile>");
   if (xurl.is_empty())
      G_THROW("Empty URL passed to DjVuFile.");
   ...
   if (!(data_pool=new DataPool(pcaster->request_data(this, url))))
      G_THROW("Failed get data for URL '"+url+"'");
   data_pool->add_trigger(-1, static_trigger_cb, this);
}
...
DjVuFile::decode_func(void)
{
   ...
   G_TRY {
	 ...
	 // Check for termination status
      ...
      {
	 GP<DjVuFile> & f=inc_files_list[pos];
	 if (f->is_decode_failed()) G_THROW("Decode of an included file failed.");
	 ...
	 if (!f->is_decode_ok())
	 {
	    ...
	    G_THROW("Internal error: an included file has not finished yet.");
	 }
      }
   } G_CATCH(exc) {
      G_TRY {
...
DjVuFile::process_incl_chunk(ByteStream & str, int file_num)
{
   ...
   {
      if (strchr(incl_str, '/'))
	 G_THROW("Malformed INCL chunk. No slashes allowed.");
   ...
   if (info)
   {
	 ...
	 if ((info->width+red-1)/red==w)
	    if ((info->height+red-1)/red==h)
	       break;
      if (red>12)
	 G_THROW("DjVu Decoder: Corrupted data (Incorrect size in BG44 chunk)\n");
      dpi=info->dpi;
   }
...
DjVuFile::decode_chunk(const char *id, ByteStream &iff, bool djvi, bool djvu, bool iw44)
{
  ...
  if (chkid == "INFO" && (djvu || djvi))
    {
      if (DjVuFile::info)
        G_THROW("DjVu Decoder: Corrupted file (Duplicate INFO chunk)");
      if (djvi)
        G_THROW("DjVu Decoder: Corrupted file (Found INFO chunk in FORM:DJVI)");
      ...
      if (info->width<0 || info->height<0)
        G_THROW("DjVu Decoder: Corrupted file (image size is zero)");
      if (info->version >= DJVUVERSION_TOO_NEW)
        G_THROW("DjVu Decoder: Cannot decode DjVu files with version >= "
  ...
  // Djbz (JB2 Dictionary)
    ...
    {
      if (DjVuFile::fgjd)
        G_THROW("DjVu Decoder: Corrupted data (Duplicate Dxxx chunk)");
      if (DjVuFile::fgjd)
        G_THROW("DjVu Decoder: Corrupted data (Dxxx chunk found after Sxxx chunk)");
  ...
  // Sjbz (JB2 encoded mask)
    ...
    {
      if (DjVuFile::fgjb)
        G_THROW("DjVu Decoder: Corrupted data (Duplicate Sxxx chunk)");
  ...
  // BG44 (background wavelets)
  else if (chkid == "BG44" && (djvu || djvi))
    {
      if (!bg44)
        {
          if (bgpm)
            G_THROW("DjVu Decoder: Corrupted data (Duplicate background layer)");
          // First chunk
  ...
  // FG44 (foreground wavelets)
    ...
    {
      if (fgpm || fgbc)
        G_THROW("DjVu Decoder: Corrupted data (Duplicate foreground color info)");
      IWPixmap fg44;
...
DjVuFile::decode(ByteStream & str)
{
   ...
   if (djvi || djvu)
     ...
     G_THROW("DejaVu decoder: a DJVU or IW44 image was expected");
   ...
   if (djvu && !info)
     G_THROW("DjVu Decoder: Corrupted data (Missing INFO chunk)");
   if (iw44 && !info)
     G_THROW("DjVu Decoder: Corrupted data (Missing IW44 data chunks)");
   if (info)
     {
       GString desc;
       if (djvu || djvi)
...
DjVuFile::get_chunk_name(int chunk_num)
{
   if(chunk_num < 0)
   {
     G_THROW("Illegal chunk number");
   }
   if((chunks_number >= 0)&&(chunk_num > chunks_number))
   {
     G_THROW("Too few chunks");
   }
   check();
...
DjVuFile::merge_anno(ByteStream &out)
{
   ...
   GP<ByteStream> str=get_merged_anno();
   if (str)
   {
      str->seek(0);
