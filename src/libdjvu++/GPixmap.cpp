...
//C-
//C- $Id: GPixmap.cpp,v 1.5.4.1 1999-03-30 21:28:24 eaf Exp $

// File "$Id: GPixmap.cpp,v 1.5.4.1 1999-03-30 21:28:24 eaf Exp $"
...
read_integer(char &c, ByteStream &bs)
{
  ...
  // check integer
  if (c<'0' || c>'9')
    THROW("Reading pixmap: integer expected");
  // eat integer
  while (c>='0' && c<='9') 
    {
      x = x*10 + c - '0';
...
color_correction_table(double gamma, unsigned char gtable[256] )
{
  ...
  if (gamma<0.1 || gamma>10.0)
    THROW("(GPixmap::color_correct) Illegal parameter");
...
GPixmap::downsample(const GPixmap *src, int factor, const GRect *pdr)
{
  ...
  if (pdr != 0)
  {
    if (pdr->xmin < rect.xmin || 
        ...
        pdr->xmax > rect.xmax || 
        pdr->ymax > rect.ymax  )
      THROW("(GPixmap::downsample) Specified rectangle overflows destination pixmap");
    rect = *pdr;
  }
...
GPixmap::upsample(const GPixmap *src, int factor, const GRect *pdr)
{
  ...
  if (pdr != 0)
  {
    if (pdr->xmin < rect.xmin || 
        ...
        pdr->xmax > rect.xmax || 
        pdr->ymax > rect.ymax  )
      THROW("(GPixmap::upsample) Specified rectangle overflows destination pixmap");
    rect = *pdr;
  }
  // initialise pixmap
  init(rect.height(), rect.width(), 0);
...
GPixmap::downsample43(const GPixmap *src, const GRect *pdr)
{
  ...
  if (pdr != 0)
  {
    if (pdr->xmin < rect.xmin || 
        ...
        pdr->xmax > rect.xmax || 
        pdr->ymax > rect.ymax  )
      THROW("(GPixmap::downsample43) Specified rectangle overflows destination pixmap");
    rect = *pdr;
...
GPixmap::upsample23(const GPixmap *src, const GRect *pdr)
{
  ...
  if (pdr != 0)
  {
    if (pdr->xmin < rect.xmin || 
        ...
        pdr->xmax > rect.xmax || 
        pdr->ymax > rect.ymax  )
      THROW("(GPixmap::upsample23) Specified rectangle overflows destination pixmap");
    rect = *pdr;
    destwidth = rect.width();
    destheight = rect.height();
  }
  ...
  if (pmr != 0)
    {
      if (pmr->xmin < rect.xmin || 
          ...
          pmr->xmax > rect.xmax || 
          pmr->ymax > rect.ymax  )
        THROW("(GPixmap::stencil) Specified rectangle overflows foreground pixmap");
      rect = *pmr;
    }
