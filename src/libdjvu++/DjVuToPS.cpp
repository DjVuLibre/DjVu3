...
DjVuToPS::Options::set_format(Format _format)
{
   if (_format!=PS && _format!=EPS)
      THROW("PostScript format must be either PS or EPS.");
...
DjVuToPS::Options::set_level(int _level)
{
   if (_level!=1 && _level!=2)
      THROW("Invalid PostScript level "+GString(_level)+". Must be either 1 or 2.");
...
DjVuToPS::Options::set_orientation(Orientation _orientation)
{
   if (_orientation!=PORTRAIT && _orientation!=LANDSCAPE)
      THROW("Invalid orientation passed to printing code.");
   if (format==EPS && _orientation==LANDSCAPE)
      THROW("LANDSCAPE mode is not supported by EPS format.");
...
DjVuToPS::Options::set_mode(Mode _mode)
{
   if (_mode!=COLOR && _mode!=FORE && _mode!=BACK && _mode!=BW)
      THROW("Invalid image mode passed to printing code.");
...
DjVuToPS::Options::set_gamma(float _gamma)
{
   if (_gamma<0.3-0.0001 || _gamma>5.0+0.0001)
      THROW("Invalid gamma specified. Must be between 0.3 and 5.0");
   gamma=_gamma;
}
		      ...
		      const GRect & prn_rect, const GRect & img_rect)
{
   ...
   if (!dimg)
      THROW("Attempt to print an empty image.");
   if (prn_rect.isempty())
      THROW("Attempt to print an empty rectangle.");
   if (img_rect.isempty())
      THROW("Attempt to scale image to point while printing.");
...
image\n",
   ...
   TRY {
	 ...
	 THROW("Not enough memory to print the image.");
		...
		const char * page_range)
{
   ...
   while(true)
   {
	 ...
	 if (ptr<dash || start_page<=0)
	    THROW("Illegal page number '"+GString(start, dash-start)+"'");
	 if (start_page>doc_pages)
	    THROW("Page number "+GString(start_page)+" is too big.");

	 if (dash<end)
	 {
	    if (dash==end-1)
	       THROW("Missing 'To' page number in range '"+
		     GString(start, end-start)+"'");
	    
	    for(ptr=dash+1;ptr<end;ptr++)
	       if (*ptr=='-')
		  THROW("Illegal range '"+GString(start, end-start)+"'");
	    
	    int end_page=strtol(dash+1, (char **) &ptr, 10);
	    if (ptr<end || end_page<=0)
	       THROW("Illegal page number '"+GString(dash+1, end-dash-1)+"'");

	    if (end_page>doc_pages)
	       THROW("Page number "+GString(end_page)+" is too big.");
   ...
   if (pages_todo.size()>1 && options.get_format()==Options::EPS)
      THROW("Can't print more than one page in EPS format.");
   ...
   for(pos=pages_todo, page_cnt=0;pos;++pos, page_cnt++)
   {
	 ...
	 if (!djvu_file->is_decode_ok())
	 {
	    ...
	    while(!djvu_file->is_decode_ok())
	    {
		  ...
		  THROW("Failed to decode page "+GString(page_num)+".");
	       if (dec_progress_cb)
      ...
      if (!dimg)
	 THROW("Failed to get image for page "+page_num);
