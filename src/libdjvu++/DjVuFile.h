...
//C-
//C- $Id: DjVuFile.h,v 1.2 1999-05-25 19:42:28 eaf Exp $
...
/** @name DjVuFile.h
    ...
    @version #$Id: DjVuFile.h,v 1.2 1999-05-25 19:42:28 eaf Exp $#
*/
...
/** #DjVuFile# plays the central role in decoding \Ref{DjVuImage}s.
    ...
    of things shared between its pages. Examples can be the navigation
    ...
    To avoid putting these chunks into every page, we have invented a new
    chunk called #INCL# which purpose is to make the decoder open the
    specified file and decode it.

    {\bf Source of data.} The #DjVuFile# does not work with hard drive only.
    Generally it asks for data through the communication mechanism provided
    by \Ref{DjVuPort} and \Ref{DjVuPortcaster}. In some cases though it's
    clear, that the source of the data is a file, and #DjVuFile# works with
...
public:
   ...
   int			file_size;
	  ...
	  @param cache Pointer to the cache of files. After decoding #DjVuFile#
	         will add itself to the cache so that it may later be
		 retrieved without decoding the file contents again */
   ...
   void		disable_standard_port(void);
	  ...
	  \begin{itemize}
	     ...
	     \item #INCL_FILES_CREATED# All #INCL# chunks have been processed
	           and the corresponding #DjVuFile#s created. This is important
		   to know to be sure that the list returned by
