...
/** @name GPixmap.h
    ...
    Instances of these class represent color images.  Each RGB pixel is
    represented by structure \Ref{Pixel}. The "bottom left" coordinate system
    ...
    {\bf ToDo} --- Several improvements have been prepared.  Support should be
    included for more sophisticated color correction schemes.  Support should
    also be added for blitting individual bitmaps with a predefined color.
    This operation should support subsampling, anti-aliasing, and sub-pixel
    positionment, just like #GBitmap::blit#.
...
public:
  ...
  /** Constructs a GPixmap by copying the rectangle #rect# of the gray level image
      #ref#.  The constructed GPixmap has the same size as rectangle #rect#.
      The pixels are initialized with shades of grays copied from #ref#. */
  ...
  /** Resets the GPixmap by copying the rectangle #rect# of the gray level
      ...
      to zero selects a linear ramp of shades of gray. */
  ...
  /** Resets the GPixmap by copying the rectangle #rect# of the gray level
      image #ref#.  The pixels are initialized with shades of grays copied
      from #ref#. */
  ...
  /** Returns the length of a row in memory.  This number is equal to the
      difference between pointers to pixels located in the same column in
      consecutive rows.  This difference may be larger than the number of
  ...
  /** Resets this GPixmap with a subsampled segment of color image #src#.
      This function conceptually subsamples image #src# by a factor #factor#,
  ...
  /** Resets this GPixmap with a oversampled segment of color image #src#.
      This function conceptually oversamples image #src# by a factor #factor#,
  ...
  /** Paints the color image #pm# through the stencil #bm# into this image.
      ...
      upsampling the GPixmap #pm# by a factor #pms# (see \Ref{upsample}),
  ...
  /** Dithers the image to 216 colors.  This function applies an ordered
      ...
      these arguments eliminates dithering artefacts on the tile
      boundaries. */
  ...
  /** Applies a luminance correction factor of #corr#.  Values greater than
      ...
      reveals unpleasant data compression artefacts.  This is why we use an
  ...
  /** Saves the image into ByteStream #bs# using the PPM format.
      Argument #raw# selects the "Raw PPM" or the "Ascii PPM" format.
      See \Ref{PNM and RLE file formats} for more information. */
