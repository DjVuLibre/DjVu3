...
// 
// $Id: DjVuDocEditor.cpp,v 1.74 2001-04-26 23:58:12 bcr Exp $
// $Name:  $
...
DjVuDocEditor::init(const GURL &url)
{
   ...
   if (!tmp_doc->is_init_ok())
      G_THROW(GUTF8String( ERR_MSG("DjVuDocEditor.open_fail") "\t")+url.get_string());
      ...
      // Moreover it will strip out any INCL chunks in that file...
{
   ...
   if (!parent_frec)
      G_THROW(GUTF8String( ERR_MSG("DjVuDocEditor.no_file") "\t")+parent_id);
   GP<DjVuFile> parent_file=get_djvu_file(parent_id);
   if (!parent_file) G_THROW(GUTF8String( ERR_MSG("DjVuDocEditor.create_fail") "\t")+parent_id);
...
//            G_THROW(GUTF8String("File '")+file_url+"' is not a DjVu file");
       ...
       G_THROW(GUTF8String( ERR_MSG("DjVuDocEditor.not_1_page") "\t")+file_url);
...
//            const GURL::Filename::UTF8 full_url(GOS::expand_name(name, GOS::url_to_filename(file_url.base())));
      ...
      gstr_out->seek(0);
      GP<DataPool> new_file_pool=DataPool::create(gstr_out);
      {
...
#endif
               if (dirurl.mkdir()<0)
                  G_THROW(GUTF8String( ERR_MSG("DjVuDocEditor.dir_fail") "\t")+dirurl);//MBCS cvt
               G_TRY {
...
DjVuDocEditor::remove_file(const GUTF8String &id, bool remove_unref)
{
   ...
   if (!djvm_dir->id_to_file(id))
      G_THROW( ERR_MSG("DjVuDocEditor.no_file") "\t"+GUTF8String(id));
                                       ...
                                       void * cl_data)
{
   ...
   str.flush();
   str.seek(0);
   GP<DataPool> file_pool=DataPool::create(gstr);
...
DjVuDocEditor::save_as(const GURL &where, bool bundled)
{
   ...
   }else
   {
     if (djvm_dir->get_files_num()==1)
     {
        ...
        doc->write(gstr);
        gstr->seek(0, SEEK_SET);
        GP<DataPool> pool=DataPool::create(gstr);
        doc_pool=pool;
        init_data_pool=pool;
