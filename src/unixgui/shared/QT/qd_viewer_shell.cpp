...
//C-
// 
// $Id: qd_viewer_shell.cpp,v 1.1 2001-08-08 17:00:32 docbill Exp $
// $Name:  $
      ...
      // DjVuViewer.
{
   ...
   {
      ThrowError("QDViewerShell::openURL", "Failed to open file '"+fname+"' for reading");
   }
...
QDViewerShell::slotGetURLTimeout(void)
{
   try
   {
	 ...
	 else
	 {
      ...
      {
        G_THROW("Remote URLs are not supported in standalone mode.");
      }
   } catch(const GException & exc)
   {
      showError(this, tr("DjVu Error"), exc);
