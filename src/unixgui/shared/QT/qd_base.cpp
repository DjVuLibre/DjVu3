...
//C-
// 
// $Id: qd_base.cpp,v 1.1 2001-05-29 22:05:30 bcr Exp $
// $Name:  $
...
QDBase::imageUpdated(void)
{
   ...
   if (dimg)
   {
      ...
      {
	 DEBUG_MSG("QDBase::imageUpdated(): decode ok\n");
	 // is_legal_bilevel() and is_legal_photo() should be mutually
	 // exclusive 
...
QDBase::exportToPNM(void)
{
   ...
   GUTF8String format_str=bw ? "PBM" : "PPM";
   GUTF8String ext=bw ? ".pbm" : ".ppm";
   
   bool raw=0;
   {
     GUTF8String mesg="This will export the DjVu image in either 'Raw "+format_str+"'\n"
     "or 'ASCII "+format_str+"' format. Which one do you prefer?\n";

     switch(QMessageBox::information(this, "Exporting DjVu image...",
       QStringFromGString(mesg),
       "&Raw", "&Ascii", "&Cancel", 0, 2))
     {
        case 0: raw=1; break;
        case 1: raw=0; break;
        default: return;
   ...
   static char * filters[]={ 0, "All files (*)", 0 };
   ...
   fd.setCaption("Select export file name...");
   ...
   if (fd.exec()==QDialog::Accepted)
   {
      ...
      QProgressDialog progress("Please stand by...",
			       "&Abort", height, this, "progress", TRUE);
      progress.setMinimumWidth(progress.sizeHint().width()*3/2);
      progress.setCaption("DjVu: Exporting image...");
	 ...
	 if (bw)
	 {
	    ...
	    if (!bmp) throw ERROR_MESSAGE("QDBase::exportToPNM",
					  "Failed to get bitmap for exporting.");
	 ...
	 } else	// not bw
	 {
	    ...
	    if (!pix) throw ERROR_MESSAGE("QDBase::exportToPNM",
					  "Failed to get pixmap for exporting.");
	    if (raw)
	    {
